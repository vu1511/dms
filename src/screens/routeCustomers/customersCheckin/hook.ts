import { GetKeyResult, PreviousData } from '@/components'
import { SwrKey } from '@/constants'
import { System } from '@/core'
import { useCurrentLocation, usePreviousRoute } from '@/hooks'
...
export const useCustomersCheckin = () => {
  ...
  const getKey = useCallback(
    (
      page: number,
      previousData: PreviousData<SearchCustomerRouteRes[]> | null
    ): GetKeyResult<SearchCustomerRouteReq> | null => {
  ...
  const fetcherHandler = useCallback(
    async ({
      ...
      type_checkin,
      hierarchy_id,
    }: GetKeyResult<SearchCustomerRouteReq>): Promise<PreviousData<SearchCustomerRouteRes[]>> => {
  ...
  const { data, hasMore }: PreviousData<SearchCustomerRouteRes[]> = useMemo(() => {
    if (!rawData?.length) {
      return { data: [], hasMore: false }
    }
  ...
  const onCreateInventory = useCallback((item: SearchCustomerRouteRes) => {
    // navigation.navigate(Routes.CreateInventory, { customer: item })
  }, [])
